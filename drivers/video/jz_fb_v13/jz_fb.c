...
int jzfb_set_par(struct fb_info *info);
static void jzfb_clk_enable(struct jzfb *jzfb);
static void jzfb_clk_disable(struct jzfb *jzfb);

#ifndef CONFIG_SLCDC_CONTINUA
void jzfb_enable_clk_auto_close(struct jzfb *jzfb);
void jzfb_disable_clk_auto_close(struct jzfb *jzfb);
#endif
					  ...
					  struct fb_info *info)
{
	...
	for (i = 0; i < jzfb->pdata->num_modes; ++i, ++mode) {
		if (mode->flag & FB_MODE_IS_VGA) {
			...
			printk("var->xres=%d var->yres=%d var->vmode=%d var->right_margin=%d\n"
				,var->xres,var->yres,var->vmode,var->right_margin);
			printk("mode->xres=%d mode->yres=%d mode->vmode=%d mode->right_margin=%d\n"
				,mode->xres,mode->yres,mode->vmode,mode->right_margin);
...
static int jzfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (mode == NULL) {
		dev_err(info->dev, "%s %d get video mode failed \n", __func__,__LINE__);
		return -EINVAL;
	}
...
//static
void slcd_send_mcu_command(struct jzfb *jzfb, unsigned long cmd)
{
	int count = 10000;
...
//add by pengweizhong
#define FIIO_CMD_LCD_ROTATE (0x36)
#define FIIO_CMD_LCD_X  (0x2a)
#define FIIO_CMD_LCD_Y  (0x2b)
#define FIIO_CMD_LCD_VDVS (0xC4)

//36H
#define FIIO_LCD_ROTATE_0 (0)
#define FIIO_LCD_ROTATE_90 (0xA0)
#define FIIO_LCD_ROTATE_180 (0xC0)
#define FIIO_LCD_ROTATE_270 (0x60)


static int fiio_lcd_rotate[4] = {0,0xA0,0xC0,0x60};
static int fiio_lcd_rotate_0_x[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};
static int fiio_lcd_rotate_0_y[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};

static int fiio_lcd_rotate_90_x[4] = {80,80,(319>> 8) & 0xff,319 & 0xff};
static int fiio_lcd_rotate_90_y[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};


static int fiio_lcd_rotate_180_x[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};
static int fiio_lcd_rotate_180_y[4] = {80,80,(319>> 8) & 0xff,319 & 0xff};

static int fiio_lcd_rotate_270_x[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};
static int fiio_lcd_rotate_270_y[4] = {0,0,(239>> 8) & 0xff,239 & 0xff};




static int g_cur_cmd = 0;
static int g_cur_data_counter = 0;
static int g_cur_lcd_rotate = 0;

//当前偏色
//值越小颜色越深,值越大颜色越浅，显示会泛白
static int g_cur_lcd_vdvs = 0x20;
...
static void jzfb_slcd_mcu_init(struct fb_info *info)
{
	...
	struct jzfb *jzfb = info->par;
	struct jzfb_platform_data *pdata = jzfb->pdata;
	//printk("%s g_cur_lcd_rotate=%d\n",__func__,g_cur_lcd_rotate);
	jzfb_1=jzfb;
    dev_err(jzfb->dev, "jzfb_slcd_mcu_init \n");
	...
	if (pdata->smart_config.length_data_table
		...
		for (i = 0; i < pdata->smart_config.length_data_table; i++) {
			...
			case SMART_CONFIG_DATA:
				...
				if (g_cur_cmd == FIIO_CMD_LCD_ROTATE) {
					//printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate[g_cur_lcd_rotate]);
					slcd_send_mcu_data(jzfb,fiio_lcd_rotate[g_cur_lcd_rotate]);
				}
				else if (g_cur_cmd == FIIO_CMD_LCD_X) {
					
					switch(g_cur_lcd_rotate) {
						case 0:
							//printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate_0_x[g_cur_data_counter]);
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_0_x[g_cur_data_counter]);
							break;
						case 1:
							//printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate_90_x[g_cur_data_counter]);
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_90_x[g_cur_data_counter]);
							break;
						case 2:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_180_x[g_cur_data_counter]);
							break;
						case 3:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_270_x[g_cur_data_counter]);
							break;
						default:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_0_x[g_cur_data_counter]);
							break;
				...
				else if (g_cur_cmd == FIIO_CMD_LCD_Y) {
					switch(g_cur_lcd_rotate) {
						case 0:
							//printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate_0_y[g_cur_data_counter]);
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_0_y[g_cur_data_counter]);
							break;
						case 1:
							//printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate_90_y[g_cur_data_counter]);
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_90_y[g_cur_data_counter]);
							break;
						case 2:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_180_y[g_cur_data_counter]);
							break;
						case 3:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_270_y[g_cur_data_counter]);
							break;
						default:
							slcd_send_mcu_data(jzfb,fiio_lcd_rotate_0_y[g_cur_data_counter]);
							break;
				...
				else if (g_cur_cmd == FIIO_CMD_LCD_VDVS) {
					slcd_send_mcu_data(jzfb,g_cur_lcd_vdvs);
				}
				else {
			...
			case SMART_CONFIG_CMD:
				...
				g_cur_cmd = pdata->smart_config.data_table[i].value;
				g_cur_data_counter = 0;
				break;
...
//static 
void jzfb_slcd_display_off(struct fb_info *info)
{
        unsigned int is_lcd_en, i,j;
        struct jzfb *jzfb = info->par;
        struct jzfb_platform_data *pdata = jzfb->pdata;
        jzfb_1=jzfb;
    	dev_err(jzfb->dev, "jzfb_slcd_display_off\n");
        if (pdata->lcd_type != LCD_TYPE_SLCD)
                return;
         ...
         * */
	if (pdata->smart_config.length_data_table
            && pdata->smart_config.off_data_table) {
                for (i = 0; i < pdata->smart_config.length_data_table; i++) {
                        switch (pdata->smart_config.off_data_table[i].type) {
                        case SMART_CONFIG_DATA:
							#if 0
                                slcd_send_mcu_data(
                                        jzfb,
                                        pdata->smart_config.off_data_table[i].value);
							#endif
							if (g_cur_cmd == FIIO_CMD_LCD_ROTATE) {
								printk("FIIO WRITE CMD=0x%02x DATA=0x%02x\n",g_cur_cmd,fiio_lcd_rotate[g_cur_lcd_rotate]);
								slcd_send_mcu_data(jzfb,fiio_lcd_rotate[g_cur_lcd_rotate]);
								...
								slcd_send_mcu_data(jzfb,pdata->smart_config.off_data_table[i].value);
							}
						
                           	break;
                        case SMART_CONFIG_CMD:
                        	slcd_send_mcu_command(jzfb,pdata->smart_config.off_data_table[i].value);
							g_cur_cmd = pdata->smart_config.off_data_table[i].value;
							g_cur_data_counter = 0;
                            break;
                        case SMART_CONFIG_UDELAY:
                                udelay(pdata->smart_config.off_data_table[i].value);
                                break;
                        default:
                                dev_err(jzfb->dev, "Unknow SLCD data type\n");
                                break;
				...
				//slcd_send_mcu_command(jzfb,0x28);
                {
                        int count = 10000;
                        while ((reg_read(jzfb, SLCDC_STATE) & SLCDC_STATE_BUSY)
                               && count--) {
...
//	printk("this once called jzfb_prepare_dma_desc() in this function::%s00000000000000\n",__func__);
	...
	dev_err(info->dev, "before mcu init\n");
	if (pdata->lcd_type == LCD_TYPE_SLCD) {
		if(!slcd_off_sign)
		jzfb_slcd_mcu_init(info);
		else
		jzfb_slcd_display_off(info);
...
static int jzfb_copy_logo(struct fb_info *info)
{
	...
	printk("jzfb_copy_logo lcd is enabled!!\n");
	jzfb->is_lcd_en = 1;
...
void test_show_logo(struct jzfb *jzfb)
{
	jzfb_set_par(jzfb->fb);
 ...
 * for test 
 */
void fiio_send_lcd_cmd(unsigned long cmd) {
	jzfb_enable(jzfb->fb);
	slcd_send_mcu_command(jzfb,cmd);
}
EXPORT_SYMBOL(fiio_send_lcd_cmd);

/*
 * for test 
 */
void fiio_send_lcd_data(unsigned long data) {
	jzfb_enable(jzfb->fb);
	slcd_send_mcu_data(jzfb,data);
}

EXPORT_SYMBOL(fiio_send_lcd_data);
...
//flag=1,偏色减小，数值增加
//value 数值增加或是减小的幅度
void fiio_debug_lcd_vdvs(int flag,int value) {
	printk(">>>>>>>>>>>>>%s last vdvs=0x%02x\n",__func__,g_cur_lcd_vdvs);
	if (0 == flag) {
		g_cur_lcd_vdvs -= value;
		if (g_cur_lcd_vdvs < 0)
			g_cur_lcd_vdvs = 0;
	}
	else {
		g_cur_lcd_vdvs += value;
		if (g_cur_lcd_vdvs > 0x3F)
			g_cur_lcd_vdvs = 0x3F;
	}
	printk(">>>>>>>>>>>>>%s current vdvs=0x%02x\n",__func__,g_cur_lcd_vdvs);
	update_lcd();
}
EXPORT_SYMBOL(fiio_debug_lcd_vdvs);
...
//2:180
//3:270
void fiio_set_lcd_rotate(int rotate) {
	int smart_ctrl = 0;
	g_cur_lcd_rotate = rotate;
	switch(g_cur_lcd_rotate) {
		case 0:
			printk("FIIO LCD WANT TO ROTATE 0.\n");
			break;
		case 1:
			printk("FIIO LCD WANT TO ROTATE 90.\n");
			break;
		case 2:
			printk("FIIO LCD WANT TO ROTATE 180.\n");
			break;
		case 3:
			printk("FIIO LCD WANT TO ROTATE 270.\n");
			break;
		default:
		break;
	}
...
EXPORT_SYMBOL(fiio_set_lcd_rotate);
int fiio_get_lcd_rotate() {
	return g_cur_lcd_rotate;
}
EXPORT_SYMBOL(fiio_get_lcd_rotate);
////////////////////////////add for change lcd rotate///////////////
struct workqueue_struct *lcd_rotate_wqueue;
struct work_struct lcd_rotate_struct_work;
static void lcd_rotate_work(struct work_struct *work) {
	int smart_ctrl = 0;
	//get current lcd rotate
	//static struct jzfb *jzfb;
	//close lcd
	...
	fb_blank(jzfb->fb, FB_BLANK_NORMAL);
	slcd_off_sign=1;
	//jzfb_display_v_color_bar(jzfb->fb);
	//jzfb_set_par(jzfb->fb);
#ifndef CONFIG_SLCDC_CONTINUA
...
static int init_lcd_rotate() {
	int ret = 0;
	INIT_WORK(&lcd_rotate_struct_work, lcd_rotate_work);
    lcd_rotate_wqueue = create_singlethread_workqueue("lcd_rotate");
    if (!lcd_rotate_wqueue) {
		printk("create_singlethread_workqueue lcd_rotate_wqueue failed!\n");
        ret = -ESRCH;
    }
	return 0;
}
...
static void update_lcd() {
	queue_work(lcd_rotate_wqueue, &lcd_rotate_struct_work);
}
////////////////////////////////////////////////////////////////////
extern void fiio_m5_disable_lcd_pwm(void);
extern void fiio_m5_enable_lcd_pwm(void);
...
#if defined(CONFIG_FB_JZ_DEBUG)
		fb_blank(jzfb->fb, FB_BLANK_UNBLANK);
		test_pattern(jzfb);
#elif defined(CONFIG_FB_JZ_LOGO)
		test_show_logo(jzfb);
		fb_blank(jzfb->fb, FB_BLANK_UNBLANK);
#endif
#ifndef CONFIG_SLCDC_CONTINUA
...
static int jzfb_suspend(struct device *dev)
{
	...
	//axp173_i2c_write_bit(axp173,0x80,0,1);
    axp173_i2c_write_bit(axp173,0x80,0,2);
    axp173_i2c_write_bit(axp173,0x80,0,3);
	printk("%s ---------------end----------------\n",__func__);
	return 0;
}
...
static int jzfb_resume(struct device *dev)
{
	...
	struct jzfb *jzfb = platform_get_drvdata(pdev);
	//normal mode 
	//if (1 == get_m5_player_mode_for_gpio_keys()) {
		mdelay(150);
	//}
	...
	#if 0 //test
	g_cur_lcd_rotate++;
	if (g_cur_lcd_rotate>4)
		g_cur_lcd_rotate = 0;
	#endif
	/* power enable */
    //axp173_i2c_write_bit(axp173,0x80,1,1);
