...
//debug
//#define FIIO_DEBUG_DWC2_GRADET
#ifdef FIIO_DEBUG_DWC2_GRADET
#define fiio_debug(x...)  printk(KERN_INFO "[fiio_dwc2_fradet] " x)
#else
#define fiio_debug(x...)
#endif
...
static void dwc2_hsotg_init_fifo(struct dwc2_hsotg *hsotg)
{
	...
	WARN(hsotg->fifo_map, "hsotg->fifo_map %x\n", hsotg->fifo_map);
	hsotg->fifo_map = 0;
			      ...
			      gfp_t gfp_flags)
{
	...
	if (hs->lx_state == DWC2_L2) {
		dev_dbg(hs->dev, "%s: don't submit request while suspended\n",
				__func__);
		fiio_debug("%s hs->lx_state == DWC2_L2 %d\n",__func__,__LINE__);
		return -EAGAIN;
	}
				      ...
				      struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
		...
		case USB_REQ_SET_ADDRESS:
			...
			dev_info(hsotg->dev, "new address %d %d\n", ctrl->wValue,__LINE__);

			m3k_usb_det=1;
			fiio_debug("%s++++++++++++++USB_REQ_SET_ADDRESS+++++++++set m3k_usb_det=1+++++++\n",__func__);
			ret = dwc2_hsotg_send_reply(hsotg, ep0, NULL, 0);
			return;

		case USB_REQ_GET_STATUS:
			fiio_debug("%s++++++++++++++USB_REQ_GET_STATUS++++++++++++++++\n",__func__);
			ret = dwc2_hsotg_process_req_status(hsotg, ctrl);
			break;

		case USB_REQ_CLEAR_FEATURE:
		case USB_REQ_SET_FEATURE:
			fiio_debug("%s++++++++++++++USB_REQ_SET_FEATURE++++++++++++++++\n",__func__);
			      ...
			      int result)
{
	...
	if (!hsotg->dedicated_fifos)
        ...
        /*
         * It's happen at id pin status change to host, device must be disconnected,
         * And we do not want has mismatch problem
         */
        id_changed = !dwc2_is_device_mode(hsotg);
        if (!id_changed) {
 ...
 */
#ifdef CONFIG_FIIO_UEVENT
	extern void fiio_remotectl_event_send(void);
#endif
static int dwc2_hsotg_corereset(struct dwc2_hsotg *hsotg)
{
	...
	if (grstctl & GRSTCTL_CSFTRST) {
		#ifdef CONFIG_FIIO_UEVENT
			//fiio_remotectl_event_send();
			printk("=====================Failed to get CSftRst asserted=========================\n");
		#endif
	...
	while (1) {
		...
		if (timeout-- < 0) {
			#ifdef CONFIG_FIIO_UEVENT
				//fiio_remotectl_event_send();
				printk("=====================USB reset failed, GRSTCTL=========================\n");
			#endif
			dev_info(hsotg->dev,
						...
						bool is_usb_reset)
{
	...
	gotgctl = dwc2_readl(hsotg->regs + GOTGCTL);       /* otg2.0 */
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	...
	epctrl |= (DXEPCTL_MPS(mps)&DXEPCTL_MPS_MASK);
...
static int dwc2_hsotg_ep_dequeue(struct usb_ep *ep, struct usb_request *req)
{
	...
	dev_dbg(hs->dev, "ep_dequeue(%p %s,%p)\n", ep, ep->name, req);
	...
	if (!on_list(hs_ep, hs_req)) {
        ...
        if (req == &hs_ep->req->req && dwc2_is_device_mode(hs))
...
#ifndef CONFIG_BOARD_HAS_NO_DETE_FACILITY
	schedule_delayed_work(&jz_info->work, msecs_to_jiffies(5000));   //Deferred execution or Repeat execution
#endif
 ...
 */
static int dwc2_hsotg_udc_stop(struct usb_gadget *gadget, struct usb_gadget_driver *driver)
{
	struct dwc2_hsotg *hsotg = to_hsotg(gadget);
	unsigned long flags = 0;
	int ep;
...
static int dwc2_hsotg_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	dev_dbg(hsotg->dev, "%s: is_on: %d op_state: %d\n", __func__, is_on,
			hsotg->op_state);
	fiio_debug("%s: is_on: %d op_state: %d\n", __func__, is_on,
			hsotg->op_state);
...
static int dwc2_hsotg_hw_cfg(struct dwc2_hsotg *hsotg)
{
	...
	hsotg->num_of_eps = (hwcfg2 >> GHWCFG2_NUM_DEV_EP_SHIFT) & 0xF;
	/* Add ep0 */
	hsotg->num_of_eps++;
	...
	for (i = 1, hwcfg1 >>= 2; i < hsotg->num_of_eps; i++, hwcfg1 >>= 2) {
		ep_type = hwcfg1 & 3;
	...
	hsotg->fifo_mem = (hwcfg3 >> GHWCFG3_DFIFO_DEPTH_SHIFT);

	hwcfg4 = dwc2_readl(hsotg->regs + GHWCFG4);
	hsotg->dedicated_fifos = (hwcfg4 >> GHWCFG4_DED_FIFO_SHIFT) & 1;
	hsotg->num_of_ineps = (hwcfg4 & GHWCFG4_NUM_IN_EPS_MASK) >> GHWCFG4_NUM_IN_EPS_SHIFT;
	hsotg->num_of_ineps++;
