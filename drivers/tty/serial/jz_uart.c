...
extern int get_fiio_m5_serial_debug(void);
...
struct uart_jz47xx_port {
	...
	struct jz_notifier serial_standby;
	suspend_state_t    cur_suspend_state;
#endif
};
...
/*
		if (uart_handle_sysrq_char(&up->port, ch))
			goto ignore_char;
*/
		if (1 == get_fiio_m5_serial_debug()) {
			if (strcmp(up->name,"uart0") == 0) {
				printk("%s[%s]: 0x%02x\n",__func__,up->name,ch);
			}
		}
...
#ifdef CONFIG_PM_SLEEP
static int serial_pm_notify(struct jz_notifier *notify,void *val)
{
	struct uart_jz47xx_port *up =
		container_of(notify, struct uart_jz47xx_port, serial_standby);
	suspend_state_t state = *(suspend_state_t  *)val;
	if(notify->msg == JZ_PM_SUSPEND_STANDBY) {
		up->cur_suspend_state = state;
	}
	return NOTIFY_OK;
}
...
static int serial_jz47xx_suspend(struct device *dev)
{
	...
	printk("device_may_wakeup(&pdev->dev) = %d\n", device_may_wakeup(dev));

	up->enable_wakeup = device_may_wakeup(dev);
	if(up->enable_wakeup && up->cur_suspend_state == PM_SUSPEND_STANDBY) {
		printk("up->enable_wakeup = %d, up->port.irq = %d, up->port.line = %d\n", up->enable_wakeup, up->port.irq, up->port.line);
...
static int serial_jz47xx_resume(struct device *dev)
{
	...
	if(up->enable_wakeup && up->cur_suspend_state == PM_SUSPEND_STANDBY) {
		...
		device_set_wakeup_enable(dev, up->enable_wakeup);
	} else {
		uart_resume_port(&serial_jz47xx_reg, &up->port);
	}
	up->cur_suspend_state = PM_SUSPEND_MAX;
...
static int serial_jz47xx_probe(struct platform_device *pdev)
{
	...
	struct jzuart_platform_data *pdata = pdev->dev.platform_data;
	int ret;
	...
	up->clk = clk_get(&pdev->dev, up->name);
	if (IS_ERR(up->clk)) {
		ret = PTR_ERR(up->clk);
		goto err_free;
	}
	...
	dma_filter = platform_get_resource(pdev, IORESOURCE_DMA, 0);
	if(dma_filter){
		up->use_dma = 1;
		up->buf_id = 0;
		...
		up->use_dma = 0;
	serial_jz47xx_ports[pdev->id] = up;
	uart_add_one_port(&serial_jz47xx_reg, &up->port);

#ifdef CONFIG_PM_SLEEP
	if(pdata && pdata->support_wakeup) {
		up->serial_standby.jz_notify = serial_pm_notify;
		up->serial_standby.level = NOTEFY_PROI_NORMAL;
		up->serial_standby.msg = JZ_PM_SUSPEND_STANDBY;
		jz_notifier_register(&up->serial_standby, NOTEFY_PROI_HIGH);
		up->enable_wakeup = pdata->support_wakeup;
		up->cur_suspend_state = PM_SUSPEND_MAX;
...
static int serial_jz47xx_remove(struct platform_device *pdev)
{
	struct uart_jz47xx_port *up = platform_get_drvdata(pdev);

	platform_set_drvdata(pdev, NULL);
#ifdef CONFIG_PM_SLEEP
	if(up->enable_wakeup) {
		jz_notifier_unregister(&up->serial_standby, NOTEFY_PROI_HIGH);
