	...
	for (i = 0; i < event->touch_point; i++) {
		...
		if (event->au16_x[i] < data->va_x_max
			...
			if(0 == i){
				...
				fiio_debug(">> 3 convert_x=%d convert_y=%d\n",convert_x,convert_y);
#endif
#endif
				if (event->touch_point == 1) {
					#if 0
					convert_x =(convert_x * 5 / 8);
					convert_y =(convert_y * 3 / 5);
					#endif
					#if 1
						if(1 == fiio_get_lcd_rotate()) {
							//90
							convert_x = event->au16_y[0];
							convert_y = 240 - event->au16_x[0];
						}	
						else if(2 == fiio_get_lcd_rotate()) {
							//180
							convert_x = event->au16_x[0];
							convert_y = event->au16_y[0];
						}
						else if(3 == fiio_get_lcd_rotate()) {
							//270
							convert_x = 240 - event->au16_y[0];
							convert_y = event->au16_x[0];
						}
						else {
							convert_x = 240 - event->au16_x[0];
							convert_y = 240 - event->au16_y[0];
						}
					#else
						s16 convert_x_0 = 0;
						s16 convert_y_0 = 0;
						convert_x_0 = 240 - event->au16_x[0];
						convert_y_0 = 240 - event->au16_y[0];
						#ifdef CONFIG_FIIO_LCD_ROTATE_90
							convert_y_0 = 240 - convert_y_0;
							convert_x = convert_y_0;
							convert_y = convert_x_0;
						#endif
					#endif
					fiio_debug(">> 4 ABS_X=%d ABS_Y=%d\n",convert_x,convert_y);
			   ...
			   const struct i2c_device_id *id)
{
	...
	uc_reg_value = FT6x06_REG_FW_VER;
