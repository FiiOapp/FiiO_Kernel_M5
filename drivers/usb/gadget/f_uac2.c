...
#define fiio_debug(x...)
#endif
#define FIIO_UAC_SAAMPLE_COUNTER (9)
/* Keep everyone on toes */
#define USB_XFERS	8


static int fiio_cur_play_rate = 0;
module_param(fiio_cur_play_rate, int, S_IRUGO);
MODULE_PARM_DESC(fiio_cur_play_rate, "FiiO M5 UAC Play Rate");
...
static void init_crate_value(int c_ssize) {
	switch(c_ssize) {
		case 2:
			...
			UAC2_RATE_VALUE_352800 = 352;
			UAC2_RATE_VALUE_384000 = 384;
			break;
		case 3:
			...
			UAC2_RATE_VALUE_352800 = 528;
			UAC2_RATE_VALUE_384000 = 576;
			break;
		case 4:
			...
			UAC2_RATE_VALUE_352800 = 704;
			UAC2_RATE_VALUE_384000 = 768;
		break;
	}
}
static int reset_uac2_rate(int freq)
{
	...
	else if (freq == UAC2_RATE_VALUE_352800) {
		actual_rate = UAC2_RATE_352800;
	}
	else if (freq == UAC2_RATE_VALUE_384000) {
		actual_rate = UAC2_RATE_384000;
	}
	else {
...
{
	struct f_uac2_opts *opts = container_of(data, struct f_uac2_opts,
					uevent_work);
        //mutex_lock(&opts->lock);
        /* send sample rate change uevent */
        send_uac2_sample_change_uevent(opts->uac2_status);
	...
	if (status)
        ...
        if (dug_uac_flags) {
			...
			if (actual_value != req->length && actual_value != 0) {
				printk("%s actual_value=%d req->length=%d mCheckUpdateSampleCounter=%d"
	            ...
	            opts->c_srate = reset_uac2_rate(actual_value);
				 fiio_cur_play_rate = opts->c_srate;
	            /* printk("%s: new opts->c_srate is  %d \n", __func__, opts->c_srate); */

	            /* send Uevent to UsbAudioClass Activity */
	            opts->uac2_status = true;
	            schedule_work(&opts->uevent_work);
	        }
	}
	substream = prm->ss;
...
uac2_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
        pr_info("%s: start or resume.\n", __func__);
		prm->ss = substream;
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
        pr_info("%s: stop or suspend.\n", __func__);
		prm->ss = NULL;
		break;
	default:
		err = -EINVAL;
...
#if defined(CONFIG_BOARD_TYPE_M3K)
...
static struct usb_string strings_fn[] = {
	...
	[STR_USB_IT].s = "M3K",
	[STR_IO_OT].s = "M3K",
	[STR_AS_OUT_ALT0].s = "M3K",
	[STR_AS_OUT_ALT1].s = "M3K",
	{ },
};
#else
static struct usb_string strings_fn[] = {
	[STR_ASSOC].s = "FiiO M5",
	[STR_IF_CTRL].s = "FiiO M5",
	[STR_CLKSRC_OUT].s = "Internal Clock",//clksrc_out,
	[STR_USB_IT].s = "FiiO M5",
	[STR_IO_OT].s = "FiiO M5",
	[STR_AS_OUT_ALT0].s = "FiiO M5",
	[STR_AS_OUT_ALT1].s = "FiiO M5",
	{ },
};
#endif
...
struct cntrl_range_lay3 {
    ...
    struct cntrl_rang_sub sub[FIIO_UAC_SAAMPLE_COUNTER];
} __packed;

static int sample_rate[FIIO_UAC_SAAMPLE_COUNTER] = {32000, 44100, 48000, 88200,
...
afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
{
	...
	if (!prm->rbuf) {
        ...
        if (!uac2_opts->uac2_binded) {
	        ret = alsa_uac2_init(agdev);
	        if (ret)
		        goto err_free_descs;
                uac2_opts->uac2_binded = true;
        }
...
static int set_support_sample_rates(void)
{
    ...
    for(i = 0; i < FIIO_UAC_SAAMPLE_COUNTER; i++)
    {
...
in_rq_range(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	...
	if (control_selector == UAC2_CS_CONTROL_SAM_FREQ) {
        cntrl_r.wNumSubRanges = FIIO_UAC_SAAMPLE_COUNTER;
...
static struct usb_function_instance *afunc_alloc_inst(void)
{
	...
	opts->uac2_status = false;
	INIT_WORK(&opts->uevent_work, uac2_sendevent_work);

	ret = misc_register(&uac2_device);
	if (ret) {
		pr_err("%s: uac2_device register failed.\n", __func__);
		return ERR_PTR(-ENOMEM);
	}

	opts->uac2_binded = false;
...
struct usb_function *afunc_alloc(struct usb_function_instance *fi)
{
	...
	++opts->refcnt;
	mutex_unlock(&opts->lock);
	fiio_debug("opts->p_chmask=%d\n",opts->p_chmask);
	fiio_debug("opts->p_srate=%d\n",opts->p_srate);
	fiio_debug("opts->p_ssize=%d\n",opts->p_ssize);
	fiio_debug("opts->c_chmask=%d\n",opts->c_chmask);
	fiio_debug("opts->c_srate=%d\n",opts->c_srate);
	fiio_debug("opts->c_ssize=%d\n",opts->c_ssize);
	agdev->func.name = "uac2";
