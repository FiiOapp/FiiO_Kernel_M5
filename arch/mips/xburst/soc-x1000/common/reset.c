...
void fiio_set_wakeup_time(int time) {
	printk("%s default:0x%08x\n",__func__,inl((RTC_IOBASE + RTC_HWFCR)));
	printk("%s %d\n",__func__,time);
	printk("%s 0x%08x\n",__func__,HWFCR_WAIT_TIME(time));
	rtc_write_reg(RTC_HWFCR, HWFCR_WAIT_TIME(time));
	printk("%s ofter:0x%08x\n",__func__,inl((RTC_IOBASE + RTC_HWFCR)));
}
EXPORT_SYMBOL(fiio_set_wakeup_time);
