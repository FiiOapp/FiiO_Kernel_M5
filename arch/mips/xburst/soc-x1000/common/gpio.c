...
#include <../chip-x1000/halley2/halley2_v10/board.h>

#define FIIO_DEBUG_GPIO
#ifdef FIIO_DEBUG_GPIO
#define fiio_debug(x...)  printk(KERN_INFO "[gpio] " x)
#else
#define fiio_debug(x...)
#endif
...
struct jzgpio_chip *slcd_jz;
struct jzgpio_chip *msc0_jz;
struct jzgpio_chip *pwm0_jz;
struct jzgpio_chip *pwm4_jz;
//struct jzgpio_chip *slcd_jz2;
struct jz_gpio_func_def *g1 = &platform_current_array[1];
struct jz_gpio_func_def *g2 = &platform_current_array[3];
struct jz_gpio_func_def *g3 = &platform_current_array[4];
struct jz_gpio_func_def *g4 = &platform_current_array[5];
//struct jzgpio_chip jz_gpio_chips[];
...
/*
static void gpio_set_func_1(struct jzgpio_chip *chip,
                          enum gpio_function func, unsigned int pins)
{
        unsigned long comp;// flags;
        unsigned long grp;
...
//add by pengweizhong
static unsigned int port_b_save[5];
void save_port_b(void) {
	struct jzgpio_chip *jz;
	jz = &jz_gpio_chips[1];
	port_b_save[0] = readl(jz->reg + PXINT);
	port_b_save[1] = readl(jz->reg + PXMSK);
	port_b_save[2] = readl(jz->reg + PXPAT1);
	port_b_save[3] = readl(jz->reg + PXPAT0);
	port_b_save[4] = readl(jz->reg + PXPEN);
	printk("BK PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	port_b_save[0],port_b_save[1],port_b_save[2],port_b_save[3],port_b_save[4]);
}
EXPORT_SYMBOL(save_port_b);

//PB04
void set_i2s_do_to_gpio(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	jz = &jz_gpio_chips[1];
	//PXINT

	//When INT n = 0: 0: Corresponding pin will be used as device function 1: Corresponding pin will be used as gpio
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0x10;
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x10;
	writel(pat1_value, jz->reg + PXPAT1);

	//PXPAT0

	//1: port PULL disable
	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value |= 0x10;
	writel(pen_value, jz->reg + PXPEN);

	printk("IO PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
}
EXPORT_SYMBOL(set_i2s_do_to_gpio);

void set_i2s_do_tofunction(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	jz = &jz_gpio_chips[1];
	#if 0
	writel(port_b_save[0], jz->reg + PXINT);
	writel(port_b_save[1], jz->reg + PXMSK);
	writel(port_b_save[2], jz->reg + PXPAT1);
	writel(port_b_save[3], jz->reg + PXPAT0);
	writel(port_b_save[4], jz->reg + PXPEN);
	#endif
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0xffffffffef;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &=0xffffffffef;
	writel(pat1_value, jz->reg + PXPAT1);

	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &=0xffffffffef;
	writel(pen_value, jz->reg + PXPEN);

	printk("I2S PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
}
EXPORT_SYMBOL(set_i2s_do_tofunction);

//set i2s port to input
//i2c 23 24
void set_i2s_to_gpio_input(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
...
EXPORT_SYMBOL(set_i2s_to_gpio_input);

void set_i2s_to_i2s_function(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0xffffffffe0;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &=0xffffffffe0;
	writel(pat1_value, jz->reg + PXPAT1);

	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &=0xffffffffe0;
	writel(pen_value, jz->reg + PXPEN);
#endif
#if 0 //i2s and i2c
	#if 0
		...
		port_b_save[1] = readl(jz->reg + PXMSK);
		msk_value = port_b_save[1];
		msk_value &=0xfe7fffe0;
		writel(msk_value, jz->reg + PXMSK);
	
		port_b_save[2] = readl(jz->reg + PXPAT1);
		pat1_value = port_b_save[2];
		pat1_value &=0xfe7fffe0;
		writel(pat1_value, jz->reg + PXPAT1);
	
		port_b_save[4] = readl(jz->reg + PXPEN);
		pen_value = port_b_save[4];
		pen_value &=0xfe7fffe0;
		writel(pen_value, jz->reg + PXPEN);
#endif
...
EXPORT_SYMBOL(set_i2s_to_i2s_function);

void set_uart0_to_gpio(void) {
	//PC10 PRUW08DGZ_G 0 1 - - - uart0_rxd(i-1) 
	//PC11 PRUW08DGZ_G 0 1 - - - uart0_txd(o)
	//PC12 PRUW08DGZ_G 0 1 - - - uart0_cts(i-0)
	//PC13 PRUW08DGZ_G 0 1 - - - uart0_rts(o) 
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	jz = &jz_gpio_chips[2];
	printk("set_uart0_to_gpio begin UART0 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |= 0x03c00;
	...
	pen_value |= 0x03c00;
	writel(pen_value, jz->reg + PXPEN);
	printk("set_uart0_to_gpio end UART0 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
}
EXPORT_SYMBOL(set_uart0_to_gpio);
void set_gpio_to_uart0(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	jz = &jz_gpio_chips[2];
	printk("set_gpio_to_uart0 begin UART0 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &= 0xffffffc3ff;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &= 0xffffffc3ff;
	writel(pat1_value, jz->reg + PXPAT1);

	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &= 0xffffffc3ff;
	writel(pen_value, jz->reg + PXPEN);

	printk("set_gpio_to_uart0 end UART0 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
}
EXPORT_SYMBOL(set_gpio_to_uart0);

void fiio_get_curent_io_state(void) {
	struct jzgpio_chip *jz;
	int i = 0;
	for(i=0;i<4;i++) {
		jz = &jz_gpio_chips[i];
		printk("PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
		readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	}
}
EXPORT_SYMBOL(fiio_get_curent_io_state);

//gpio status
//static unsigned int g_fiio_porta_state[5];
//static unsigned int g_fiio_portb_state[5];
//i2c 23 24
void set_i2si2c0_to_gpio_input(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	#if 0
	jz = &jz_gpio_chips[0];
	g_fiio_porta_state[0] = readl(jz->reg + PXINT);
	g_fiio_porta_state[1] = readl(jz->reg + PXMSK);
	g_fiio_porta_state[2] = readl(jz->reg + PXPAT1);
	g_fiio_porta_state[3] = readl(jz->reg + PXPAT0);
	g_fiio_porta_state[4] = readl(jz->reg + PXPEN);
	printk("I2S-SAVE PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	jz = &jz_gpio_chips[1];
	g_fiio_portb_state[0] = readl(jz->reg + PXINT);
	g_fiio_portb_state[1] = readl(jz->reg + PXMSK);
	g_fiio_portb_state[2] = readl(jz->reg + PXPAT1);
	g_fiio_portb_state[3] = readl(jz->reg + PXPAT0);
	g_fiio_portb_state[4] = readl(jz->reg + PXPEN);
	printk("I2S-SAVE PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	#endif
	jz = &jz_gpio_chips[1];
...
EXPORT_SYMBOL(set_i2si2c0_to_gpio_input);

void set_i2si2c0_to_function(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
...
#if 0
	jz = &jz_gpio_chips[0];
	writel(g_fiio_porta_state[0], jz->reg + PXINT);
	writel(g_fiio_porta_state[1], jz->reg + PXMSK);
	writel(g_fiio_porta_state[2], jz->reg + PXPAT1);
	writel(g_fiio_porta_state[3], jz->reg + PXPAT0);
	writel(g_fiio_porta_state[4], jz->reg + PXPEN);
	printk("I2S-OK PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));

	jz = &jz_gpio_chips[1];
	writel(g_fiio_portb_state[0], jz->reg + PXINT);
	writel(g_fiio_portb_state[1], jz->reg + PXMSK);
	writel(g_fiio_portb_state[2], jz->reg + PXPAT1);
	writel(g_fiio_portb_state[3], jz->reg + PXPAT0);
	writel(g_fiio_portb_state[4], jz->reg + PXPEN);
	printk("I2S-OK PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
#endif
}
EXPORT_SYMBOL(set_i2si2c0_to_function);
/**
 *设置LED为高阻状态
 */
 void fiio_set_led_to_input_no_pull_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0x00600020;
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x00600020;
	writel(pat1_value, jz->reg + PXPAT1);
	...
	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value |= 0x00600020;
	writel(pen_value, jz->reg + PXPEN);
 }
EXPORT_SYMBOL(fiio_set_led_to_input_no_pull_state);
 
 void fiio_set_led_to_normal_function_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0xFF9FFFDF;
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &=0xFF9FFFDF;
	writel(pat1_value, jz->reg + PXPAT1);
	...
	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &=0xFF9FFFDF;
	writel(pen_value, jz->reg + PXPEN);
 }
EXPORT_SYMBOL(fiio_set_led_to_normal_function_state);


/**
 * 进入到蓝牙接收模式时，设置按键IO DAC使能IO的IO状态为输入 无上拉下拉状态 
 * key(PB28 PB31) LED(PB05 PB21 PB22) ----DAC(PA16) key(PA19)
 * PB
 * 00000000(31-24) 00000000(23-16) 00000000(15-8) 00000000(7-0)
 * 10010000        01100000        00000000       00100000 
 * 90              60              00             20 
 * PA
 * 00000000        00001001        00000000       00000000 
 * 00              09              00              00   
 */
 void fiio_set_io_to_input_no_pull_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	jz = &jz_gpio_chips[0];
	printk("GPIO PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0x00090000;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x00090000;
	writel(pat1_value, jz->reg + PXPAT1);
	...
	//PXINT
 	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0x90600020;
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x90600020;
	writel(pat1_value, jz->reg + PXPAT1);
...
/**
 * 进入到蓝牙接收模式时，设置按键IO DAC使能IO的IO状态为输入 无上拉下拉状态 
 * key(PB28[vol-] PB31[wakup]) LED(PB05 PB21 PB22) ----DAC(PA16[po_en]) key(PA19[vol+])
 * PB
 * 00000000(31-24) 00000000(23-16) 00000000(15-8) 00000000(7-0)
 * PXMSK
 * 10010000        01100000        00000000       00100000 
 ...
 * 00              09              00              00   
 */
 void fiio_set_io_to_normal_gpio_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	//PXINT
	port_b_save[0] = readl(jz->reg + PXINT);
	msk_value = port_b_save[0];
	//io as gpio
	msk_value |=0x00080000; //b19 - 1
	msk_value &=0xFFFEFFFF;//b16 - 0
	writel(msk_value, jz->reg + PXINT);
	
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	//io as gpio
	msk_value &=0xFFF7FFFF;//b19 - 0
	msk_value |=0x00010000; //b16 - 1
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	//b19-in
	pat1_value |=0x00080000; //b19 - 1 Edge trigger interrupt
	//b16-out
	pat1_value &=0xFFFEFFFF; //b16 - 0 Corresponding pin is used as gpio output
	writel(pat1_value, jz->reg + PXPAT1);


	port_b_save[3] = readl(jz->reg + PXPAT0);
	pen_value = port_b_save[3];
	//b19 - 0 Port is falling edge triggered interrupt input
	//b16 - 0 Port is GPIO output 0
	pen_value &= 0xFFF6FFFF;//b16 b19 - 0
	writel(pen_value, jz->reg + PXPAT0);
	
	//0: port PULL enable
	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &= 0xFFF6FFFF;
	writel(pen_value, jz->reg + PXPEN);
	printk("GPIO PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
	...
	msk_value = port_b_save[0];
	//io as gpio
	msk_value |=0x90000000; //b28 b31 - 1
	msk_value &=0xFF9FFFDF;//b5 b21 b22 - 0
	writel(msk_value, jz->reg + PXINT);

	//PXMSK
 	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0x6FFFFFFF;//b28 b31 - 0
	msk_value |=0x00600020; //b5 b21 b22 - 1
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x90000000; //b28 b31 - 1 Edge trigger interrupt
	pat1_value &=0xFF9FFFDF; //b5 b21 b22  - 0 Corresponding pin is used as gpio output
	writel(pat1_value, jz->reg + PXPAT1);

	//PXPAT0
	port_b_save[3] = readl(jz->reg + PXPAT0);
	pen_value = port_b_save[3];
	//b28 b31  - 0 Port is falling edge triggered interrupt input
	//b5 b21 b22 - 0 Port is GPIO output 0
	pen_value &= 0x6F9FFFDF;//b28 b31 b5 b21 b22 - 0
	writel(pen_value, jz->reg + PXPAT0);
	
	//1: port PULL disable
	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &= 0x6F9FFFDF;
	writel(pen_value, jz->reg + PXPEN);
	printk("I2S PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
	readl(jz->reg + PXINT),readl(jz->reg + PXMSK),readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));
 }
...
/**
 * 进入到蓝牙接收模式时，设置I2C I2S 控制IO 按键IO DAC使能IO的IO状态为输入 无上拉下拉状态 
 * I2S(PB00 PB01 PB02 PB03 PB04) I2C(PB23 PB24) key(PB28 PB31) LED(PB05 PB21 PB22) ----DAC(PA16) key(PA19)
 * PB
 * 00000000(31-24) 00000000(23-16) 00000000(15-8) 00000000(7-0)
 * 10010001        11100000        00000000       00111111 
 * 91              E0              00             3F 
 * PA
 * 00000000        00001001        00000000       00000000 
 * 00              09              00              00   
 */
 void fiio_set_gpio_to_input_no_pull_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0x91E0003F;
	writel(msk_value, jz->reg + PXMSK);

	//When INT n = 0 and PMSK = 1 (GPIO function): 0: Corresponding pin is used as gpio output 1: Corresponding pin is used as gpio input
	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0x91E0003F;
	writel(pat1_value, jz->reg + PXPAT1);
 ...
 EXPORT_SYMBOL(fiio_set_gpio_to_input_no_pull_state);
 /**
  *当退出蓝牙接收模式时，设置为正常的模式
  * I2S(PB00 PB01 PB02 PB03 PB04) I2C(PB23 PB24) key(PB28 PB31) LED(PB05 PB21 PB22) DAC(PA16) key(PA19)
  * PB
  * 00000000(31-24) 00000000(23-16) 00000000(15-8) 00000000(7-0)
  * 10010001		11100000		00000000	   00111111 
  * 0110 1110        00011111        1111 1111       1100 0000
  * 6E              1F              FF             C0 
  * PA
  * 00000000        00001001        00000000       00000000 
  * 11111111        11110110        11111111       11111111
  * FF              F6              FF             FF   
  */
  
  void fiio_set_gpio_to_normal_function_state(void) {
	struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0x6E1FFFC0;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &=0x6E1FFFC0;
	writel(pat1_value, jz->reg + PXPAT1);
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &=0xFFF6FFFF;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &=0xFFF6FFFF;
	writel(pat1_value, jz->reg + PXPAT1);
 ...
 EXPORT_SYMBOL(fiio_set_gpio_to_normal_function_state);

 void fiio_init_gpio_for_shutdown(void) {
	 struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value |=0xFFFFFFFF;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value |=0xFFFFFFFF;
	writel(pat1_value, jz->reg + PXPAT1);

	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value |= 0xFFFFFFFF;
	writel(pen_value, jz->reg + PXPEN);
	#if 0
 	//PB
	jz = &jz_gpio_chips[1];
 ...
 EXPORT_SYMBOL(fiio_init_gpio_for_shutdown);

  void fiio_init_gpio_for_resume(void) {
	 struct jzgpio_chip *jz;
	unsigned int msk_value = 0;
	unsigned int pat1_value = 0;
	unsigned int pen_value = 0;
	...
	port_b_save[1] = readl(jz->reg + PXMSK);
	msk_value = port_b_save[1];
	msk_value &= 0x00000000;
	writel(msk_value, jz->reg + PXMSK);

	port_b_save[2] = readl(jz->reg + PXPAT1);
	pat1_value = port_b_save[2];
	pat1_value &= 0x00000000;
	writel(pat1_value, jz->reg + PXPAT1);

	port_b_save[4] = readl(jz->reg + PXPEN);
	pen_value = port_b_save[4];
	pen_value &= 0x00000000;
	writel(pen_value, jz->reg + PXPEN);
 ...
 EXPORT_SYMBOL(fiio_init_gpio_for_resume);
void fiio_resume_gpio(void)
{
	int i;
	struct jzgpio_chip *jz;
...
EXPORT_SYMBOL(fiio_resume_gpio);
//保存当前采样率
static int g_fiio_m5_play_sample = 0;
void set_fiio_m5_play_sample(int sample) {
    g_fiio_m5_play_sample = sample;
}
EXPORT_SYMBOL(set_fiio_m5_play_sample);
int get_fiio_m5_play_sample(void) {
    return g_fiio_m5_play_sample;
}
EXPORT_SYMBOL(get_fiio_m5_play_sample);
 ...
 *1:bt sink
 */
static int tmp_fiio_m5_player_mode = 0;
void set_m5_player_mode_for_gpio_keys(int mode) {
    tmp_fiio_m5_player_mode = mode;
}
EXPORT_SYMBOL(set_m5_player_mode_for_gpio_keys);

int get_m5_player_mode_for_gpio_keys(void) {
    return tmp_fiio_m5_player_mode;
}
EXPORT_SYMBOL(get_m5_player_mode_for_gpio_keys);
/*
 *区别当前是spdif还是I2S模式
 *0:SPDIF  1:I2S  default:0
 */
static int g_fiio_spdif_or_i2s_port_mode = 0;
void set_fiio_spdif_or_i2s_port_mode(int mode) {
    g_fiio_spdif_or_i2s_port_mode = mode;
}
EXPORT_SYMBOL(set_fiio_spdif_or_i2s_port_mode);
int get_fiio_spdif_or_i2s_port_mode(void) {
    return g_fiio_spdif_or_i2s_port_mode;
}
EXPORT_SYMBOL(get_fiio_spdif_or_i2s_port_mode);


/*
 * 0:local play,ak4377 output
 * 1:local play,bt output
 */
static int g_fiio_m5_output_select_bt = 0;
void set_fiio_m5_output_select_bt(int flag) {
    g_fiio_m5_output_select_bt = flag;
}
EXPORT_SYMBOL(set_fiio_m5_output_select_bt);
int get_fiio_m5_output_select_bt(void) {
    return g_fiio_m5_output_select_bt;
}
EXPORT_SYMBOL(get_fiio_m5_output_select_bt);

/**
 * 1:serial debug
 * 0:serial no debug
 */
static int g_fiio_m5_serial_debug_flag = 0;
void set_fiio_m5_serial_debug(int flag) {
    g_fiio_m5_serial_debug_flag = flag;
}
EXPORT_SYMBOL(set_fiio_m5_serial_debug);
int get_fiio_m5_serial_debug(void) {
    return g_fiio_m5_serial_debug_flag;
}
EXPORT_SYMBOL(get_fiio_m5_serial_debug);

#define FIIO_M5_RESET_GPIO_FOR_BT_SINK (1)
#define FIIO_X1000_GPIO_PORT_A (0)
#define FIIO_X1000_GPIO_PORT_B (1)
#define FIIO_X1000_GPIO_PORT_C (2)
#define FIIO_X1000_GPIO_PORT_D (3)

//#define FIIO_TEST_SYS_SUSPEND 1
void gpio_suspend_set(struct jzgpio_chip *jz,int port_type)
{
	unsigned int d,grp;
    unsigned int pxint,pxmsk,pxpat1,pxpat0,pxpen,pxpen_last;
	unsigned long flags;
    ...
    fiio_debug("save  PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x d=0x%08x\n",
        jz->save[0],jz->save[1],jz->save[2],jz->save[3],jz->save[4],d);
	...
	pxpat0 = jz->save[3] & d;
	pxpen = (~jz->save[4]) & d;
    fiio_debug("00 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x \n",
        pxint,pxmsk,pxpat1,pxpat0,~pxpen);
    ...
    pxpen_last = ~pxpen;
    fiio_debug("01 PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x PXPEN=0x%08x ~d=0x%08x\n",
        pxint,pxmsk,pxpat1,pxpat0,pxpen_last,(~d));

#ifdef FIIO_M5_RESET_GPIO_FOR_BT_SINK
    fiio_debug(">>>>>>>>>>>%s port_type=%d mode=%d\n",__func__,port_type,get_m5_player_mode_for_gpio_keys());
    if (port_type == FIIO_X1000_GPIO_PORT_B) {
        if (FIIO_M5_PLAYER_BT_SINK == get_m5_player_mode_for_gpio_keys()) {
            //I2C LED I2S nopull
            pxint &= ~0x01E00037; //0: Corresponding pin is used as device functions or normal gpio
            pxmsk |= 0x01E00037;  //1: Corresponding pin will be used as gpio
            pxpat1 |= 0x01E00037; //1: Corresponding pin is used as gpio input
            pxpen_last |= 0x01E00037; //1: port PULL disable(no pull)
            fiio_debug("fiio set i2s io to input no pull state in bt sink mode!\n");
            fiio_debug("fiio set led(PB05 PB21 PB22) to input no pull state in bt sink mode!\n");
            fiio_debug("fiio set i2c(PB23 PB24) to input no pull state in bt sink mode!\n");
        }
    }

    if (port_type == FIIO_X1000_GPIO_PORT_C) {
        //uart
         if (FIIO_M5_PLAYER_BT_SINK == get_m5_player_mode_for_gpio_keys()) {
             pxint &= ~0x00003C00; //0: Corresponding pin is used as device functions or normal gpio
             pxmsk |= 0x00003C00; //1: Corresponding pin will be used as gpio
             pxpat1 |= 0x00003C00;  //1: Corresponding pin is used as gpio input
             pxpen_last |= 0x00003C00; //1: port PULL disable(no pull)
             fiio_debug("fiio set uart to input no pull state in bt sink mode!\n");
         }
    }

    if (port_type == FIIO_X1000_GPIO_PORT_D) {
        if (FIIO_M5_PLAYER_NORMAL == get_m5_player_mode_for_gpio_keys()) {
            pxint &= 0xFFFFFFF7; //0: Corresponding pin is used as device functions or normal gpio
            pxmsk |= 0x00000008; //1: Corresponding pin will be used as gpio
            pxpat1 &= 0xFFFFFFF7; //0: Corresponding pin is used as gpio output
            pxpat0 &= 0xFFFFFFF7; //0: Port is GPIO output 0
            pxpen_last |= 0x00000008; //1: port PULL disable
            fiio_debug("fiio set spdif[0]/i2s[1] to gpio output low in normal mode!\n");
        }
        else {
            if (FIIO_M5_AUDIO_PATH_SPDIF == get_fiio_spdif_or_i2s_port_mode()) {
                pxint &= 0xFFFFFFF7; //0: Corresponding pin is used as device functions or normal gpio
                pxmsk |= 0x00000008; //1: Corresponding pin will be used as gpio
                pxpat1 &= 0xFFFFFFF7; //0: Corresponding pin is used as gpio output
                pxpat0 &= 0xFFFFFFF7; //0: Port is GPIO output 0
                pxpen_last |= 0x00000008; //1: port PULL disable
                fiio_debug("fiio set spdif[0]/i2s[1] to gpio output low in bt sink spdif mode!\n");
            }
            else {
                pxint &= 0xFFFFFFF7; //0: Corresponding pin is used as device functions or normal gpio
                pxmsk |= 0x00000008; //1: Corresponding pin will be used as gpio
                pxpat1 &= 0xFFFFFFF7; //0: Corresponding pin is used as gpio output
                pxpat0 |= 0x00000008; //1: Port is GPIO output 1
                pxpen_last &= 0xFFFFFFF7; //0: port PULL enable
                fiio_debug("fiio set spdif[0]/i2s[1] to gpio output high in bt sink i2s mode!\n");
            }
        }
    }
#endif

    fiio_debug("write PXINT=0x%08x PXMSK=0x%08x PXPAT1=0x%08x PXPAT0=0x%08x pxpen_last=0x%08x \n",
        pxint,pxmsk,pxpat1,pxpat0,pxpen_last);

	writel(pxint,jz->reg + PXINT);
	writel(pxmsk,jz->reg + PXMSK);
	writel(pxpat1,jz->reg + PXPAT1);
    writel(pxpat0,jz->reg + PXPAT0);
    writel(pxpen_last,jz->reg + PXPEN);
	
    fiio_debug("pxint:0x%08x,pxmsk:0x%08x,pxpat1:0x%08x,pxpat0:0x%08x,pxpen_last:0x%08x\n"
          ,readl(jz->reg + PXINT),readl(jz->reg + PXMSK),
           readl(jz->reg + PXPAT1),readl(jz->reg + PXPAT0),readl(jz->reg + PXPEN));

	spin_unlock_irqrestore(&jz->gpio_lock,flags);
}

void gpio_suspend_set_1(struct jzgpio_chip *jz)
{
        unsigned int d,grp;
		...
		writel(0x0,jz->reg + PXINT);
        writel(0xffffffff,jz->reg + PXMSK);
		writel(0xffffffff,jz->reg + PXPAT1);
		writel(0xffffffff,jz->reg + PXPEN);
/*
	writel(0xffff,jz->reg + PXINT);
        writel(0xffff,jz->reg + PXMSK);
        writel(0xffff,jz->reg + PXPAT1);
        writel(0xffff,jz->reg + PXPAT0);
        writel(0xffff,jz->reg + PXPEN);
*/  
      //writel(0x0,jz->reg + PXPAT0);
        //gpio_set_func_1(pwm0_jz, g3->func, g3->pins);
        //gpio_set_func_1(pwm4_jz, g4->func, g4->pins);
...
int __init setup_gpio_pins(void)
{
	...
	//struct jz_gpio_func_def *g1 = &platform_lcd_array[0];
    	slcd_jz = &jz_gpio_chips[g1->port];
	msc0_jz = &jz_gpio_chips[g2->port];
	pwm0_jz = &jz_gpio_chips[g3->port];
	pwm4_jz = &jz_gpio_chips[g4->port];
	//slcd_jz2 = &jz_gpio_chips[g2->port];
...
subsys_initcall(setup_gpio_pins);
/* -------------------------gpio proc----------------------- */
#include <jz_proc.h>
