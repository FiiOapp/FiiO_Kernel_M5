...
static struct cgu_clk cgu_clks[] = {
	[CGU_AUDIO_I2S] = {CPM_I2SCDR, 1<<29, 0x1f << 13, 13, 0x1fff, 0, SELECTOR_AUDIO},//0x1f 13
...
static int cgu_audio_enable(struct clk *clk,int on)
{
	...
	if(on){
		...
		if (no == CGU_AUDIO_I2S) {
			...
			printk("not setting cgu_i2s, please check !\n");
			dump_stack();
		}
		cgu_clks[no].cache = 0;
	}else{
...
extern int get_fiio_spdif_or_i2s_port_mode(void);
static int cgu_audio_calculate_set_rate(struct clk* clk, unsigned long rate, unsigned int pid){
	...
	for(i=0; i < ARRAY_SIZE(audio_rate); i++){
		/* MCLK = 4*BCLK = 4*64*LRCLK, LRCLK=Sameple Rate */
		if(audio_div[i].rate == rate/256)
			break;
	}
...
//#ifdef CONFIG_SND_ASOC_JZ_AIC_SPDIF_V13
		...
		if (0 == get_fiio_spdif_or_i2s_port_mode()) {
			printk(">>>>File:%s Function:%s m *= 2\n",__FILE__,__func__);
			m *= 2;
		}
		
#else
	#ifdef CONFIG_SND_ASOC_JZ_AIC_SPDIF_V13
		printk(">>>>File:%s Function:%s m *= 2 LINE=%d\n",__FILE__,__func__,__LINE__);
		m *= 2;
	#endif
#endif
		d_max = 0x1ff;
		tmp_val = audio_div[i].saven / 256;
		if(tmp_val > n_max){
		...
		if(tmp_val&cgu_clks[no].en){
			...
			if (no == CGU_AUDIO_I2S) {
				/* write access I2SCDR1 to trigger M/N enable*/
				cpm_outl(cpm_inl(cgu_clks[CGU_AUDIO_I2S1].off), cgu_clks[CGU_AUDIO_I2S1].off);
			} else {
				printk("not setting cgu_i2s, please check !\n");
				dump_stack();
			}
		...
		if(tmp_val&cgu_clks[no].en){
			...
			if (no == CGU_AUDIO_PCM) {
				/* write access I2SCDR1 to trigger M/N enable*/
				cpm_outl(cpm_inl(cgu_clks[CGU_AUDIO_PCM1].off), cgu_clks[CGU_AUDIO_PCM1].off);
			} else {
				printk("not setting cgu_pcm, please check !\n");
				dump_stack();
			}
		}else{
			cgu_clks[no].cache = tmp_val;
...
struct audio_div_values divvalues_tmp[ARRAY_SIZE(audio_rate)] = {
//struct audio_div_values divvalues[2] = {
#ifdef FIIO_USE_HIGH_SAMPLE
...
static  void get_audio_div(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(divvalues); i ++) {
		if(a < ARRAY_SIZE(audio_rate)) {
			...
			printk("audiodiv_apll[%d].rate = %d, audiodiv_apll[%d].savem =  %d, audiodiv_apll[%d].saven = %d\n",
			 	a, audiodiv_apll[a].rate, a, audiodiv_apll[a].savem, a, audiodiv_apll[a].saven);
			a++;
		} else {
...
#else
			audiodiv_mpll[m].rate = audio_rate[m];
			audiodiv_mpll[m].savem = divvalues_tmp[m].savem;
			audiodiv_mpll[m].saven = divvalues_tmp[m].saven;
#endif
			printk("audiodiv_mpll[%d].rate = %d, audiodiv_mpll[%d].savem =  %d, audiodiv_mpll[%d].saven = %d\n",
			 	m, audiodiv_mpll[m].rate, m, audiodiv_mpll[m].savem, m, audiodiv_mpll[m].saven);
			m++;
		}
	}
