...
static int jz_aic_probe(struct platform_device *pdev)
{
	...
	/* the following code move to arch/mips/xburst/soc-4775/common/clk.c::cgu_enable() */
#if 0 // defined(CONFIG_SOC_4775)
...
static void jz_aic_release(struct device *dev)
{
	printk("releasing '%s'\n", dev_name(dev));
}
...
static int jz_asoc_aic_init(void)
{
	int err = 0;
		...
		err = platform_driver_register(&jz_asoc_aic_driver);
		if (err)
			return err;

		/* Register snd_uac2 device */
		err = platform_device_register(&jz_aic_device);
		if (err) {
...
static void jz_asoc_aic_exit(void)
{
	#ifdef CONFIG_FIIO_SOUND_BUILD_MODULE
		platform_driver_unregister(&jz_asoc_aic_driver);
		platform_device_unregister(&jz_aic_device);
	#else
		platform_driver_unregister(&jz_asoc_aic_driver);
	#endif
}
