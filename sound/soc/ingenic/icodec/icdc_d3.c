...
EXPORT_SYMBOL(ak4376_power_mode);
#endif
...
extern void AK4377_WriteOneByte_Volume(uint8 WriteAddr,uint8 DataToWrite);
extern void fiio_set_dsd_volume(unsigned char m_volume);
						 ...
						 unsigned int value)
{
    ...
    if(reg == AK4376_DACL || reg == AK4376_DACR || reg == AK4376_BALANCE) {
		...
		if (reg == AK4376_DACL || reg == AK4376_DACR) {
			if (0 == val) {
				//mute
				AK4377_Mute();
			}
		   ...
		   volumeR=ak4376_volume[val];
			...
			val = value % 2;
          	if(val != 0) {
				if(work_sign)
				{
					printk("try to set ak437x low mode\n");
					#ifdef CONFIG_SND_ASOC_FIIO_AK4376
					...
					//ak4377_power_mode=1;
				#endif
				printk("start ak437x low mode\n");
			}
			else if(val==0) {
				if(work_sign) {
					printk("try to set ak437x high mode\n");
					#ifdef CONFIG_SND_ASOC_FIIO_AK4376
					...
					//ak4377_power_mode=0;
				#endif
				printk("start ak437x high mode\n");
			}  
        }
 	} 
	else if(reg == AK4376_VOLUME) {
		printk(">>>>>>>>want>>>>>>%s reg = 0x%x value = 0x%x val=%x >>>>>>>>>\n",__func__,reg,value,val);
		//tinymix 12 (0~59)
		// value 0x3C~0x01  0x7C
		int m_volume = 0x3C-value;// 0~60
		if(value>0xBC)
			m_volume = 0xFC-value;
		else if(value>0x7C)
			m_volume = 0xBC-value;
...
/* unit: 0.01dB */
static const DECLARE_TLV_DB_SCALE(ak4377_t1v, -4800, 80, 0);
...
static const struct snd_kcontrol_new icdc_d3_snd_controls[] = {
	...
	SOC_SINGLE_TLV("AK4377 Playback Volume",AK4376_VOLUME,0, 60, 1, ak4377_t1v),
...
static ssize_t icdc_d3_powerdown_dac_store(struct device *dev, struct device_attribute *attr,
								  const char *buf, size_t count)
{
	struct icdc_d3 *icdc_d3 = dev_get_drvdata(dev);
	struct snd_soc_codec *codec = icdc_d3->codec;
	printk("begin......................icdc_d3_powerdown_dac_store........................\n");
	snd_soc_update_bits(codec, SCODA_REG_AICR_ADC, SCODA_AICR_ADC_SB_MASK, 1<<SCODA_AICR_ADC_SB_SHIFT);
	snd_soc_update_bits(codec, SCODA_REG_AICR_DAC, SCODA_AICR_DAC_SB_MASK, 1<<SCODA_AICR_DAC_SB_SHIFT);
	snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_SLEEP_MASK, 1<<SCODA_CR_VIC_SB_SLEEP_SHIFT);
	snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_MASK, 1<<SCODA_CR_VIC_SB_SHIFT);
	icdc_d3_set_bias_level(codec, SND_SOC_BIAS_OFF);
	snd_soc_update_bits(codec, SCODA_REG_CR_CK, SCODA_CR_CK_SDCLK_MASK, 1<<SCODA_CR_CK_SDCLK_SHIFT);
	printk("end......................icdc_d3_powerdown_dac_store........................\n");
	return count;
}


static ssize_t icdc_d3_powerup_dac_store(struct device *dev, struct device_attribute *attr,
								  const char *buf, size_t count)
{
	struct icdc_d3 *icdc_d3 = dev_get_drvdata(dev);
	struct snd_soc_codec *codec = icdc_d3->codec;
	printk("begin......................icdc_d3_powerup_dac_store........................\n");
	snd_soc_update_bits(codec, SCODA_REG_CR_CK, SCODA_CR_CK_SDCLK_MASK, 0<<SCODA_CR_CK_SDCLK_SHIFT);
	icdc_d3_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	if (snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_MASK, 0<<SCODA_CR_VIC_SB_SHIFT))
		msleep(250);
	...
	snd_soc_update_bits(codec, SCODA_REG_AICR_ADC, SCODA_AICR_ADC_SB_MASK, 0<<SCODA_AICR_ADC_SB_SHIFT);
	snd_soc_update_bits(codec, SCODA_REG_AICR_DAC, SCODA_AICR_DAC_SB_MASK, 0<<SCODA_AICR_DAC_SB_SHIFT);
	printk("end......................icdc_d3_powerup_dac_store........................\n");
	return count;
}

static DEVICE_ATTR(powerdown_dac, S_IWUSR | S_IRUSR,
				   NULL, icdc_d3_powerdown_dac_store);

static DEVICE_ATTR(powerup_dac, S_IWUSR | S_IRUSR,
				   NULL, icdc_d3_powerup_dac_store);
static struct attribute *icdc_d3_attributes[] = {
	&dev_attr_hw_regs.attr,
	&dev_attr_powerdown_dac.attr,
	&dev_attr_powerup_dac.attr,
	NULL
};
...
static void jz_icdc_release(struct device *dev)
{
	printk("releasing '%s'\n", dev_name(dev));
}
...
static int icdc_d3_modinit(void)
{
	int err = 0;
		...
		err = platform_driver_register(&icdc_d3_codec_driver);
		if (err)
			return err;

		/* Register snd_uac2 device */
		err = platform_device_register(&jz_icdc_device);
		if (err) {
...
static void icdc_d3_exit(void)
{
	...
	#ifdef CONFIG_FIIO_SOUND_BUILD_MODULE
		platform_driver_unregister(&icdc_d3_codec_driver);
		platform_device_unregister(&jz_icdc_device);
	#else
		platform_driver_unregister(&icdc_d3_codec_driver);
	#endif
}
