...
//debug
//#define FIIO_DEBUG_SPDIF
#ifdef FIIO_DEBUG_SPDIF
#define fiio_debug(x...)  printk(KERN_INFO "[fiio_spdif] " x)
#else
#define fiio_debug(x...)
#endif
...
static unsigned long  spdif_select_ori_sample_freq(struct device *aic,unsigned long sync)
{
	...
	fiio_debug("%s div=0x%02x sample=%d\n",__func__,div,sync);
...
static unsigned long  spdif_select_sample_freq(struct device* aic,unsigned long sync)
{
	...
	fiio_debug("%s div=0x%02x sync=%d\n",__func__,div,sync);
...
static int jz_spdif_set_rate(struct device *aic ,struct jz_aic* jz_aic, unsigned long sample_rate){
	...
	fiio_debug("%s +++++++000++++++++sample_rate=%d\n",__func__,sample_rate);
	clk_set_rate(cgu_aic_clk, sample_rate*64);        //sample_rate * 256  / 4 
	fiio_debug("%s +++++++001++++++++\n",__func__);
	...
	__i2s_start_bitclk(aic);
#endif
	spdif_select_ori_sample_freq(aic,sample_rate);    //sample_rate * 256  / 4
...
//		__i2s_set_transmit_trigger(aic,trigger/2);
		__spdif_set_trigger(aic, fls(trigger / 4) - 1);
		fiio_debug("%s +++++++001++++++++\n",__func__);
	...
	/* sample rate */
	fiio_debug("%s +++++++001++++++++ sample_rate=%d\n",__func__,sample_rate);
...
static int jz_spdif_probe(struct snd_soc_dai *dai)
{
	...
	/*select spdif trans*/
	fiio_debug("spdif cpu dai prob ok\n");
#endif
	return 0;
}
...
{
	fiio_debug(">>>>File:%s Function:%s\n",__FILE__,__func__);
	set_fiio_spdif_or_i2s_port_mode(0);
