...
#ifdef CONFIG_SND_ASOC_FIIO_PCM5242
...
static int jz_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_I2S:		/*i2s format*/
		#if 0
		if (1 == get_fiio_m5_output_select_bt())
			__i2s_select_msb_fmt(aic);
		else
	...
	case SND_SOC_DAIFMT_CBM_CFM:	/*sync and bit clk (codec master : i2s slave)*/
		...
		I2S_DEBUG_MSG("%s %d SND_SOC_DAIFMT_CBM_CFM.\n",__func__,__LINE__);
		break;
	case SND_SOC_DAIFMT_CBS_CFM:
		...
		I2S_DEBUG_MSG("%s %d SND_SOC_DAIFMT_CBS_CFM.\n",__func__,__LINE__);
		break;
	}
		...
		unsigned int freq, int dir)
{
	...
	{	
		I2S_DEBUG_MSG("%s %d SND_SOC_CLOCK_OUT.\n",__func__,__LINE__);
		//__i2s_stop_bitclk(aic);
		//__i2s_bclk_output(aic);
		//__i2s_sync_output(aic);
		//__i2s_start_bitclk(aic);
...
*/
extern void set_fiio_m5_play_sample(int sample);
...
//	clk_srcs[CLK_ID_MPLL].rate = 600000000;
 	...
 	I2S_DEBUG_MSG("%s:sample_rate=%lu,channels=%d fmt_width=%d\n"
	...
	*/
	...
	clk_srcs[CLK_ID_MPLL].rate = 400000000;
        ...
        __i2s_set_dv(aic, (0));
	break;*/
	case 176400: //294/15625 ~ 294/15624=42/2232  blck=64fs
	sample_sign=13;
	//	clk_srcs[CLK_ID_MPLL].rate = 599961600;
	//audio_write(0,I2SCDR_PRE);
	audio_write((3<<29)|(28<<13)|(1250<<0),I2SCDR_PRE); //1008MHz
	audio_write(0,I2SCDR1_PRE);
	__i2s_set_dv(aic, (1)); //sys_clk=128fs
	break;
	case 192000:  // sys_clock = 12288000 = 64fs
	//600000000*64/3125/64=12288000/64=192000
	sample_sign=14;
	// audio_write((3<<29)|(384<<13)|(7875<<0),I2SCDR_PRE);
	// audio_write(0,I2SCDR1_PRE);
	// __i2s_set_dv(aic, (3));//256fs
	//600000000*64/3125/64=12288000/64=192000
	audio_write((7<<29)|(64<<13)|(3125<<0),I2SCDR_PRE);
	audio_write(0,I2SCDR1_PRE);
	...
	#ifdef CONFIG_SND_ASOC_FIIO_AK4377
        set_fiio_m5_play_sample(sample_sign);
		ak4377_sample_set(sample_sign,1);
	#endif
	#ifdef CONFIG_SND_ASOC_FIIO_AK4376
...
extern void fiio_set_dac_volume();
		...
		struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		#ifdef CONFIG_SND_ASOC_FIIO_AK4377
			//AK4377_WriteOneByte(0x0B,128);
			//AK4377_WriteOneByte(0x0C,0);
			AK4377_Mute();
	...
	I2S_DEBUG_MSG("Exit %s, substream = %s\n",
				__func__,
				(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? "playback" : "capture");
	return;
}
...
static int jz_i2s_probe(struct snd_soc_dai *dai)
{
	...
	printk(">>>>File:%s Function:%s v001\n",__FILE__,__func__);
	
	I2S_DEBUG_MSG("enter %s\n", __func__);
	//add by pengweizhong
	#if 1
	//解决从spdif模式切换回到I2S模式时，I2S没有输出的问题
	__spdif_disable(aic);
	__spdif_disable_transmit_dma(aic);
	
	__interface_select_i2s(aic); //important
	
	__i2s_enable_record(aic);
	__i2s_enable_replay(aic);
	//open it will lead to record disable
	//__i2s_enable_loopback(aic);
	__i2s_sync_output(aic);
	__i2s_bclk_output(aic);
	__i2s_start_bitclk(aic);
...
//add for BT sink play
struct jz_i2s *g_jz_i2s;
int fiio_set_i2s_hw_params(int sample_rate,int channels,int fmt_width)
{
	struct device *aic = g_jz_i2s->aic;
	enum dma_slave_buswidth buswidth;
	int trigger;
	struct clk *clk_srcs = get_clk_from_id(0);
    I2S_DEBUG_MSG("%s:sample_rate=%lu,channels=%d fmt_width=%d\n"
		,__func__,sample_rate,channels, fmt_width);
	if (channels > 2) {
		printk("hw params not inval channel %d \n",
				channels);
		return -EINVAL;
	}
	//playback
	...
	g_jz_i2s->tx_dma_data.buswidth = buswidth;
	g_jz_i2s->tx_dma_data.max_burst = (I2S_TFIFO_DEPTH * buswidth)/2;
	trigger = I2S_TFIFO_DEPTH - (g_jz_i2s->tx_dma_data.max_burst/(int)buswidth);
	__i2s_set_oss(aic, fmt_width);
	__i2s_set_transmit_trigger(aic, (trigger/2));
	//delete by pengweizhong only here
	//snd_soc_dai_set_dma_data(dai, substream, (void *)&g_jz_i2s->tx_dma_data);
	switch(sample_rate){
		/*case 6000:
		audio_write((7<<29)|(4<<13)|(3125<<0),I2SCDR_PRE);
	audio_write(0,I2SCDR1_PRE);
...
EXPORT_SYMBOL(fiio_set_i2s_hw_params);
...
void fiio_start_i2s() {
	...
	work_mode = aic_set_work_mode(aic, work_mode, true);
	if (work_mode != AIC_I2S_MODE) {
		dev_warn(g_jz_i2s->aic, "Aic now is working on %s mode, open i2s mode failed\n",
				aic_work_mode_str(work_mode));
		return -EPERM;
	}
	if (!g_jz_i2s->i2s_mode) {
		__aic_select_i2s(aic);
		__i2s_play_lastsample(aic);
		__i2s_set_transmit_trigger(aic, I2S_TFIFO_DEPTH/4);
		__i2s_set_receive_trigger(aic, (I2S_RFIFO_DEPTH/4 - 1));
	...
	//default
	fiio_set_i2s_hw_params(48000,2,16);
}
EXPORT_SYMBOL(fiio_start_i2s);

void fiio_stop_i2s() {
	int timeout = 150000;
	struct device *aic = g_jz_i2s->aic;
	enum aic_mode work_mode = AIC_I2S_MODE;
	work_mode = aic_set_work_mode(g_jz_i2s->aic, work_mode, false);
	g_jz_i2s->i2s_mode &= ~I2S_WRITE;
	if (!g_jz_i2s->i2s_mode)
		__aic_disable(aic);
	//play maybe can't use it
	if (__i2s_transmit_dma_is_enable(aic)) {
		__i2s_disable_transmit_dma(aic);
		__aic_clear_tur(aic);
		/*hrtime mode: stop will be happen in any where, make sure there is
...
static int jz_i2s_init(void)
{
	...
	printk(">>>>File:%s Function:%s\n",__FILE__,__func__);
	set_fiio_spdif_or_i2s_port_mode(1);
