...
static void jz_aic_dma_release(struct device *dev)
{
	printk("releasing '%s'\n", dev_name(dev));
}
...
static int jz_pcm_init(void)
{
	...
	#ifdef CONFIG_FIIO_SOUND_BUILD_MODULE
		printk(">>>>File:%s Function:%s mode\n",__FILE__,__func__);
		err = platform_driver_register(&jz_pcm_platfrom_driver);
		if (err)
			return err;

		/* Register snd_uac2 device */
		err = platform_device_register(&jz_aic_dma_device);
		if (err) {
...
static void jz_pcm_exit(void)
{
	#ifdef CONFIG_FIIO_SOUND_BUILD_MODULE
		platform_driver_unregister(&jz_pcm_platfrom_driver);
		platform_device_unregister(&jz_aic_dma_device);
	#else
		platform_driver_unregister(&jz_pcm_platfrom_driver);
	#endif
}
